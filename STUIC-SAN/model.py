...
class Model():
    def __init__(self, usernum, itemnum, timenum, args, reuse=None):
        ...
        self.geo_matrix = tf.placeholder(tf.int32, shape=(None, args.maxlen, args.maxlen))
        ...
        self.geo_matrix = tf.reshape(self.geo_matrix, [tf.shape(self.input_seq)[0], args.maxlen, args.maxlen])
        with tf.variable_scope("POIRec", reuse=reuse):
            ...
            # geo K V
            geo_matrix_emb_K = embedding(
                self.geo_matrix,
                vocab_size=args.geo_span + 1,
                num_units=args.hidden_units,
                zero_pad=False,
                scale=False,
                l2_reg=args.l2_emb,
            ...
            time_matrix_emb_V = tf.layers.dropout(time_matrix_emb_V,
                                                  rate=args.dropout_rate,
                                                  training=tf.convert_to_tensor(self.is_training))
            geo_matrix_emb_K = tf.layers.dropout(geo_matrix_emb_K,
                                                 rate=args.dropout_rate,
                                                 training=tf.convert_to_tensor(self.is_training))
            geo_matrix_emb_V = tf.layers.dropout(geo_matrix_emb_V,
                                                 rate=args.dropout_rate,
                                                 training=tf.convert_to_tensor(self.is_training))
            ...
            for i in range(args.num_blocks):
                with tf.variable_scope("num_blocks_%d" % i):
                    ...
                    self.seq = multihead_attention(queries=normalize(self.seq),
                                                   ...
                                                   geo_matrix_K=geo_matrix_emb_K,
                                                   geo_matrix_V=geo_matrix_emb_V,
        ...
        self.loss = tf.reduce_sum(- tf.log(tf.sigmoid(self.pos_logits) + 1e-24) * istarget - tf.log(
            1 - tf.sigmoid(self.neg_logits) + 1e-24) * istarget) / tf.reduce_sum(istarget)
        ...
        if reuse is None:
            ...
            self.optimizer = tf.train.AdamOptimizer(learning_rate=args.lr, beta2=0.999)
    ...
    def predict(self, sess, u, seq, time_matrix, geo_matrix, item_idx):
        return sess.run(self.test_logits,
                        {self.u: u, self.input_seq: seq, self.time_matrix: time_matrix, self.geo_matrix: geo_matrix,
