...
def multihead_attention(queries,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        geo_matrix_K_ = tf.concat(tf.split(geo_matrix_K, num_heads, axis=3), axis=0)
        geo_matrix_V_ = tf.concat(tf.split(geo_matrix_V, num_heads, axis=3), axis=0)
        ...
        outputs_geo = tf.squeeze(tf.matmul(geo_matrix_K_, tf.expand_dims(Q_, axis=3)))

        outputs = outputs + outputs_time + outputs_pos + outputs_geo
        ...
        outputs_value = tf.matmul(outputs, V_)
        outputs_pos_value = tf.matmul(outputs, absolute_pos_V_)
        output_geo_value = tf.reshape(tf.matmul(tf.expand_dims(outputs, axis=2), geo_matrix_V_),
                                      [tf.shape(outputs_pos)[0], tf.shape(outputs_pos)[1], num_units])
        output_time_value = tf.reshape(tf.matmul(tf.expand_dims(outputs, axis=2), time_matrix_V_),
                                       [tf.shape(outputs_pos)[0], tf.shape(outputs_pos)[1], num_units])

        outputs = outputs_value + output_time_value + outputs_pos_value + output_geo_value
